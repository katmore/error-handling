...
[ "$ECS_OPTS" ] || ECS_OPTS='--no-progress-bar --no-interaction --fix'
...
if [ "$HELP_MODE" ]; then
   ...
   echo "Operands:"
   echo "  SOURCE:"
   echo "    Optionally specify source path(s) to format."
   echo "    Default: '$SOURCE'"
   echo 
...
$ECS_BIN $ECS_OPTS check $SOURCE && {
  [ -n "$QUIET" ] || echo "$ME: 'easy-coding-standard' format successful"
} || {
  lint_status=$?
  >&2 echo "$ME: 'easy-coding-standard' format failed with status $lint_status"
